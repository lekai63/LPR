...
//  }
//
// 增加isUpdate参数，默认false,即insert到数据库；传入true，则做update
func ExportToSQL(ctx context.Context, db execContexter, df *dataframe.DataFrame, tableName string, isUpdate bool, options ...SQLExportOptions) error {
	...
	for row := start; row <= end; row++ {
		...
		if batchSize != nil && batchCount == *batchSize {
			...
			err := sqlInsert(ctx, isUpdate, db, database, tableName, columnNames, batchData)
			if err != nil {
				return err
			}
...
func sqlInsert(ctx context.Context, isUpdate bool, db execContexter, database Database, tableName string, columnNames []string, batchData []interface{}) error {
	...
	if isUpdate {
		updateFields := []string{
			"accrued_principal=(tmp.accrued_principal::BIGINT)",
			"plan_date=(tmp.plan_date::date)",
			"plan_interest=(tmp.plan_interest::BIGINT)",
			"plan_amount=(tmp.plan_amount::BIGINT)",
		}
		// 组装批量更新语句，更新accrued_principal
		stmt = "UPDATE " + tableName + " SET " + strings.Join(updateFields, ",") + " FROM ( VALUES " + placeholders + " ) as tmp " + "( " + columns + ") " + "WHERE " + tableName + ".id=(tmp.id::INTEGER)"
	} else {
